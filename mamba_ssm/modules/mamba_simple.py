...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if inference_params is not None:
            ...
            if inference_params.seqlen_offset > 0:
                ...
                out, _, ssm_state = self.step(hidden_states, conv_state, ssm_state)
        ...
        # self.in_proj.weight @ rearrange(hidden_states, "b l d -> d (b l)")的结果是4d (b l)
        # xz的尺寸为b 4d l
        if self.in_proj.bias is not None:
            xz = xz + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")

        A = -torch.exp(self.A_log.float())  # A的尺寸为(d_inner, d_state)，或者说为4d 16
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            out = self.out_proj(y)
        return out
    def _mamba_step(self, A, x_s, dt_s, B_s, C_s, z_s, ssm_state):
        """ 单步计算封装函数，用于梯度检查点 """
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        x = self.conv1d(x.transpose(1,2)).transpose(1,2) # (B S D)
