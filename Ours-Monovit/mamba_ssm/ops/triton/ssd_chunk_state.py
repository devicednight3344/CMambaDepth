...
def _chunk_cumsum_fwd_kernel(
    ...
    # dt_ptr最终指向(chunk_size, nheads)的矩阵
    dt_out_ptr += pid_b * stride_dt_out_batch + pid_c * stride_dt_out_chunk
    # dt_out_ptr最终指向一个尺寸为(nheads, chunk_size)的矩阵
    # 虽然dt_out_ptr的初始计算跳过了nheads维度，但在实际操作时，比如在后续的tl.store或者tl.load过程中，
    # nheads的处理是通过线程的并行性隐式处理的。
    # 实际的nheads维度偏移是在加载或存储时通过offs_h进行的。例如，dt_out_ptrs = dt_out_ptr +
    # (offs_h[:, None] * stride_dt_out_head + offs_c[None, :] * stride_dt_out_csize)
    # 这一步就会根据每个线程处理的nheads和chunk_size进行细粒度的内存操作。
    dA_cumsum_ptr += pid_b * stride_dA_cs_batch + pid_c * stride_dA_cs_chunk
    # dA_cumsum_ptr最终指向一个尺寸为(nheads, chunk_size)的矩阵
    offs_h = pid_h * BLOCK_SIZE_H + tl.arange(0, BLOCK_SIZE_H)
    offs_c = tl.arange(0, BLOCK_SIZE_CHUNK)
    dt_ptrs = dt_ptr + (offs_h[:, None] * stride_dt_head + offs_c[None, :] * stride_dt_seqlen)
    # 这个dt_ptrs特别巧妙，dt_ptr最终指向(chunk_size, nheads)的矩阵，但是dt_ptrs却指向了(nheads, chunk_size)的矩阵，
    # 虽然偏移：(offs_h[:, None] * stride_dt_head + offs_c[None, :] * stride_dt_seqlen)会使得它隔几个数据取出一个数，
    # 但是它最后取出来的数据刚刚好是dt_ptr指向的矩阵作了转置的结果（如果BLOCK_SIZE_H和BLOCK_SIZE_CHUNK刚好等于nheads, chunk_size，
    # 最终dt_ptrs指向的矩阵尺寸为(BLOCK_SIZE_H,BLOCK_SIZE_CHUNK)
    A_ptrs = A_ptr + offs_h * stride_A_head
    # 最终A_ptrs指向的矩阵尺寸为(BLOCK_SIZE_H,)
    dt_out_ptrs = dt_out_ptr + (offs_h[:, None] * stride_dt_out_head + offs_c[None, :] * stride_dt_out_csize)
    # 最终dt_out_ptrs指向的矩阵尺寸为(BLOCK_SIZE_H,BLOCK_SIZE_CHUNK)
    dA_cs_ptrs = dA_cumsum_ptr + (offs_h[:, None] * stride_dA_cs_head + offs_c[None, :] * stride_dA_cs_csize)
    # 最终dA_cs_ptrs指向的矩阵尺寸为(BLOCK_SIZE_H,BLOCK_SIZE_CHUNK)
    chunk_size_limit = min(chunk_size, seqlen - pid_c * chunk_size)

    dt = tl.load(dt_ptrs, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit), other=0.0).to(tl.float32)
    # dt是一个(BLOCK_SIZE_H, BLOCK_SIZE_CHUNK)的矩阵
    if HAS_DT_BIAS:
        # dt_bias_ptr指向的矩阵尺寸为(nheads,)
        dt_bias = tl.load(dt_bias_ptr + offs_h * stride_dt_bias_head, mask=offs_h < nheads, other=0.0).to(tl.float32)
        # dt_bias是一个尺寸为(BLOCK_SIZE_H,)的矩阵
        dt += dt_bias[:, None]
    if DT_SOFTPLUS:
        dt = softplus(dt)
    ...
    # dt_limit = (0.0, float("inf"))，因此dt_min=0.0，dt_max=float("inf")
    dt = tl.where((offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit), dt, 0.0)
    # (offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit)最终得到一个(BLOCK_SIZE_H, BLOCK_SIZE_CHUNK)的bool矩阵
    # 最终dt是一个(BLOCK_SIZE_H, BLOCK_SIZE_CHUNK)的矩阵
    tl.store(dt_out_ptrs, dt, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size))
    A = tl.load(A_ptrs, mask=offs_h < nheads, other=0.0).to(tl.float32)
    # A的矩阵尺寸为(BLOCK_SIZE_H,)
    dA = dt * A[:, None]
...
def _chunk_state_fwd_kernel(
    ...
    # b_ptr指向尺寸为(chunk_size, d_state)的矩阵
    x_ptr += pid_b * stride_x_batch + pid_c * chunk_size * stride_x_seqlen + pid_h * stride_x_head
    # x_ptr指向尺寸为(chunk_size, headdim)的矩阵
    dt_ptr += pid_b * stride_dt_batch + pid_c * stride_dt_chunk + pid_h * stride_dt_head
    # dt_ptr指向尺寸为(chunk_size,)的矩阵
    dA_cumsum_ptr += pid_b * stride_dA_cs_batch + pid_c * stride_dA_cs_chunk + pid_h * stride_dA_cs_head
    # dA_cumsum_ptr指向尺寸为(chunk_size,)的矩阵
    if HAS_SEQ_IDX:
        seq_idx_ptr += pid_b * stride_seq_idx_batch + pid_c * chunk_size * stride_seq_idx_seqlen
    # BLOCK_SIZE_M对应headdim维度，BLOCK_SIZE_N对应d_state维度，BLOCK_SIZE_K对应chunk_size维度
    offs_m = pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
    offs_n = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
    offs_k = tl.arange(0, BLOCK_SIZE_K)
    x_ptrs = x_ptr + (offs_m[:, None] * stride_x_hdim + offs_k[None, :] * stride_x_seqlen)
    # x_ptrs指向尺寸为(BLOCK_SIZE_M,BLOCK_SIZE_K)的矩阵（(headdim, chunk_size)）
    b_ptrs = b_ptr + (offs_n[None, :] * stride_b_dstate + offs_k[:, None] * stride_b_seqlen)
    # b_ptrs指向尺寸为(BLOCK_SIZE_K,BLOCK_SIZE_N)的矩阵（(chunk_size, d_state)）
    dt_ptrs = dt_ptr + offs_k * stride_dt_csize
    # dt_ptrs指向尺寸为(BLOCK_SIZE_K,)的矩阵（(chunk_size,)）
    dA_cs_last = tl.load(dA_cumsum_ptr + (chunk_size - 1) * stride_dA_cs_csize).to(tl.float32)
    # dA_cs_last是一个数
    dA_cumsum_ptrs = dA_cumsum_ptr + offs_k * stride_dA_cs_csize
    # dA_cumsum_ptrs指向尺寸为(BLOCK_SIZE_K,)的矩阵（(chunk_size,)）
    if HAS_SEQ_IDX:
    ...
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        # k=0、BLOCK_SIZE_K、2*BLOCK_SIZE_K...
        ...
        # dA_cs_k是一个(BLOCK_SIZE_K,)的向量（(chunk_size,)）
        if HAS_SEQ_IDX:
        ...
        # dt_k是一个(BLOCK_SIZE_K,)的向量（(chunk_size,)）
        if not HAS_SEQ_IDX:
        ...
        acc += tl.dot(x, b)
        # acc是一个(BLOCK_SIZE_M,BLOCK_SIZE_N)的矩阵
        x_ptrs += BLOCK_SIZE_K * stride_x_seqlen
        # x_ptrs指向尺寸为(BLOCK_SIZE_M,BLOCK_SIZE_K)的矩阵（(headdim, chunk_size)），一共有nchunks个线程，每个
        # 线程都处理一个(headdim, chunk_size)的矩阵，现在某个线程带着自己的(headdim, chunk_size)矩阵进入现在这个for循环
        # 本行代码：x_ptrs += BLOCK_SIZE_K * stride_x_seqlen，隐含着将chunk_size分成数个BLOCK_SIZE_K，每次for循环的
        # 迭代都从原本的(headdim, chunk_size)大矩阵里切出(BLOCK_SIZE_M,BLOCK_SIZE_K)的小矩阵，迭代的末尾通过
        # x_ptrs += BLOCK_SIZE_K * stride_x_seqlen确定下一次的(BLOCK_SIZE_M,BLOCK_SIZE_K)小矩阵应该取哪些位置的值
    ...
    # 和acc一样，states是一个(BLOCK_SIZE_M,BLOCK_SIZE_N)的矩阵
