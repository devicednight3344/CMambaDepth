...
# from mmcv.runner import load_checkpoint,load_state_dict # 由于mmcv的版本太新，runner已经不在mmcv里了
# 需要改成from mmengine.runner import load_checkpoint,load_state_dict
from mmengine.runner import load_checkpoint,load_state_dict
from mmcv.cnn import build_norm_layer

# from mmseg.utils import get_root_logger # 高版本mmseg.utils里没有get_root_logger，改成import logging
import logging
...
class MPViT(nn.Module):
    ...
    def forward_features(self, x, pos_encoding):

        # x's shape : [B, C, H, W]
        outs = []
        x = self.stem(x)  # Shape : [B, C, H/2, W/2]
        # x的尺寸为torch.Size([1, 64, 96, 320])
    ...
    def forward(self, x, pos_encoding):
        x = self.forward_features(x, pos_encoding)
