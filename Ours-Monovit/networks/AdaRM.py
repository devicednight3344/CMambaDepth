...
class LKA(nn.Module):
    def __init__(self, dim, DDW_k):
        super().__init__()
        self.conv0 = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)
        DDW_padding = (DDW_k - 1) * 3 // 2
        self.conv_spatial = nn.Conv2d(dim, dim, DDW_k, stride=1, padding=DDW_padding, groups=dim, dilation=3)
...
class Attention(nn.Module):
    def __init__(self, d_model, DDW_k):
        super().__init__()

        self.proj_1 = nn.Conv2d(d_model, d_model, 1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = LKA(d_model, DDW_k)
...
class RetBlock(nn.Module):
    ...
    def forward(
        ...
        if self.layerscale:
            ...
            # x = self.drop_path(self.gamma_2 * self.ffn(self.final_layer_norm(x)))
        else:
            ...
            # x = self.drop_path(self.ffn(self.final_layer_norm(x)))
        return x
...
class ConvBn2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1):
        ...
        self.bn = nn.BatchNorm2d(out_channels)  # BN 归一化

    def forward(self, x):
        return self.bn(self.conv(x))  # 先卷积再 BN

class LargeKernel(nn.Module):
    def __init__(self, channels, kernel, small_kernels, groups):
        super(LargeKernel, self).__init__()
        self.dw_large = ConvBn2d(channels, channels, kernel, padding=kernel // 2, groups=groups)
        self.small_kernels = small_kernels
        for k in self.small_kernels:
            setattr(self, f"dw_small_{k}", ConvBn2d(channels, channels, k, padding=k // 2, groups=groups))
...
class LargeKernelBlock(nn.Module):
    def __init__(self, channels, kernel, small_kernels, dw_ratio=1.0, activation=nn.ReLU, drop_path=0.,
                 mlp_ratio=4.0, groups=None):
        ...
        self.dw = LargeKernel(int(channels * dw_ratio), kernel, small_kernels=small_kernels, groups=groups)
...
class SpatialAttention(nn.Module):
    def __init__(self, dim, kernel_size, out_channel, expand_ratio=2):
        super().__init__()
        ...
        self.proj = nn.Conv2d(dim, out_channel, 1)
    def forward(self, x):
        B, C, H, W = x.shape
        x = self.norm(x)
...
class AdaRM(nn.Module):
    def __init__(self, in_channels, num_heads, Ret_depths, LK_depths, LK_dims, kernel_sizes, groups,
                 Ret_drop_path_rate=0.1, LK_drop_path_rate=0.3, squeeze_ratio=4, layer_init_values=1e-6,
                 drop_path_rate=0.05, layerscale=False, use_checkpoint=True, index=None, DDW_k=7):
        super().__init__()
        ...
        # mlp_ratio = 4.0
        # small_kernels = (3,)
        # assert isinstance(LK_depths, tuple)
        # self.layer_norm_LK = nn.ModuleDict()
        # self.gamma_LK = nn.ParameterDict()
        # self.LK_blocks = nn.ModuleDict()
        # for stage, (depth, dim, ksize, group) in enumerate(zip(LK_depths, LK_dims, kernel_sizes, groups)):
        #     dpr = (x for x in np.linspace(0, LK_drop_path_rate, depth))
        #     self.LK_blocks["{}".format(stage)] = nn.Sequential()
        #     for _ in range(depth):
        #         self.LK_blocks["{}".format(stage)].add_module(str(_), LargeKernelBlock(dim, ksize, small_kernels=small_kernels,
        #                        dw_ratio=dw_ratio, mlp_ratio=mlp_ratio, drop_path=next(dpr), groups=group))
        #     self.layer_norm_LK["{}".format(stage)] = LayerNorm(dim, eps=1e-6, data_format="channels_first")
        #     self.gamma_LK["{}".format(stage)] = nn.Parameter(layer_init_values * torch.ones((1,dim,1,1)),
        #                               requires_grad=True)
            # if stage < len(LK_depths) - 1:
            #     self.LK_blocks.append(DownSample(dim, LK_dims[stage + 1]))
        self.LK_blocks = Attention(in_channels,DDW_k)
        ...
        # self.conv_modulation = SpatialAttention(in_channels*(len(LK_depths) + 1), 3, out_channel=in_channels)
        self.out_project = ConvBlock(in_channels, in_channels)
        # self.out_project = ConvBlock1x1(in_channels, in_channels)
        # self.gamma = nn.Parameter(torch.ones((in_channels,)))  # Global learnable scale
        # torch.save(self.conv_modulation.state_dict(), "D:\题目资料\单目/tmp/002.pt")
        # pretrained_dict = torch.load("D:\题目资料\单目/tmp/002.pt")
    ...
    def forward(self, x):
        ...
        # local CNN
        # LK_xs = []
        # for branch in range(len(self.LK_blocks)):
        #     LK_x = x
        #     for blk in self.LK_blocks["{}".format(branch)]:
        #         LK_x = blk(LK_x)
        #     LK_x = self.layer_norm_LK["{}".format(branch)](LK_x) * self.gamma_LK["{}".format(branch)]
        #     LK_xs.append(LK_x)
        # LK_x = LK_xs[0]
        # for LK_x_middle in LK_xs[1:]:
        #     LK_x = LK_x + LK_x_middle
...
if __name__ == "__main__":
    ...
    a = AdaRM(72,4,Ret_depths,LK_depths,LK_dims,kernel_sizes)
    a.to(device)
