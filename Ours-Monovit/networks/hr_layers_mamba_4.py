...
class UpsampleConv(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        super(UpsampleConv, self).__init__(
            ConvBNELU(in_channels, out_channels),
...
class Mamba_4(nn.Module):
    ...
    def forward(self, hidden_states, inference_params, height, width, record_out):
        ...
        if record_out:
            ...
            ssm_state = self.step(hidden_states, conv_state, ssm_state, record_out)
            # step函数的算法跟mamba的常规算法是一样的,只不过要求token数必须为1
            return ssm_state
    ...
    def step(self, hidden_states, conv_state, ssm_state, height, width, record_out):
        ...
        feature = rearrange(feature, '(b h w) D -> b h w D', b=B/(height*width), h=height, w=width)
        feature = feature.permute(0, 3, 1, 2).contiguous()
        x = self.conv2d(feature)
        x = self.act(x).to(dtype=dtype)
        x = rearrange(x, 'b D h w -> b h w D', b=B/(height*width), h=height, w=width)
        x = rearrange(x, 'b h w (S D) -> B S D', B=B, b=B / (height * width), h=height, w=width,
                      S=S, D=D)
...
class MambaModule_4(nn.Module):
    def __init__(self, input_channels, hide_channel, d_model, mamba_sqe, mamba_sqe_list, mamba_channel_list,
                 linear_in_channel, out_channel, out_channels, out_sqe_num, out_mamba_sqe_num, d_model_origin, downsample_size,
                 d_conv=1, dt_rank=2, layer_idx=0):
        ...
        self.mamba = nn.ModuleDict()
        for i in range(len(mamba_sqe_list)):
            self.mamba["{}".format(i)] = Mamba_4(d_model=d_model, seqlen=mamba_sqe, d_conv=d_conv, dt_rank=dt_rank,
                                                 layer_idx=layer_idx)
        self.layer_norm_2 = nn.LayerNorm(d_model)
        self.convs["decoder_embedding"] = nn.Conv2d(d_model*out_sqe_num, linear_in_channel, 3, padding=1,
                                                    groups=out_mamba_sqe_num)
...
class fSEModule_2(nn.Module):
    def __init__(self, high_feature_channel, low_feature_channels, out_channel):
    ...
    def forward(self, high_feature, low_feature, use_high_feature):
        if use_high_feature:
            features = [upsample(high_feature)]
            features += low_feature
            features = torch.cat(features, 1)
            features = self.conv1x1(features)
            features = self.conv3x3(features)
        else:
            features = self.conv1x1(low_feature)
            features = self.conv3x3(features)
        return features