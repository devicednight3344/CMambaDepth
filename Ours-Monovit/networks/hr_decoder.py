...
#                  num_output_channels=1, use_channel_mamba=False, use_channel_mamba_2=False, use_channel_mamba_3=False):
...
#                 if self.use_channel_mamba:
#                     # torch.save(features["X_{}{}".format(row + 1, col - 1)],
#                     #            "D:\题目资料\单目\余弦相似度测试图片/02/X_16weights_{}_98.pt".format(index))
...
#                         features_map = torch.cat(low_features, 1)
#                     torch.save(torch.cat([features_map, high_feature], 1),
#                                "D:\题目资料\单目\余弦相似度测试图片/02/X_16weights_{}_88.pt".format(index))
#                     # 12x384x24x80
...
#                     torch.save(features["X_" + index], "D:\题目资料\单目\余弦相似度测试图片/02/X_16weights_{}_11.pt".format(index))
#                 else:
#                     # torch.save(features["X_{}{}".format(row + 1, col - 1)],
#                     #            "D:\题目资料\单目\余弦相似度测试图片/08/X_16weights_{}_98.pt".format(index))
...
#                     # torch.save(features["X_" + index], "D:\题目资料\单目\余弦相似度测试图片/08/X_16weights_{}_12.pt".format(index))
...
from collections import OrderedDict
from .hr_layers import *
from networks.HAM import AdaRM
...
class DepthDecoder(nn.Module):
    def __init__(self, ch_enc=[64, 128, 216, 288, 288], scales=range(4), num_ch_enc=[64, 64, 128, 256, 512],
        ...
        if use_AdaRM:
            ...
            kernel_sizes = [(3,), (3,)]
            groups = [(12,), (12,)]
            self.convs["AdaRM_d3_0"] = AdaRM(in_channels[0], num_heads[0],Ret_depths[0], LK_depths[0], LK_dims[0],
                                             kernel_sizes[0], groups[0], index="d3_0")
            self.convs["AdaRM_d4_0"] = AdaRM(in_channels[1], num_heads[1], Ret_depths[1], LK_depths[1], LK_dims[1],
                                             kernel_sizes[1], groups[1], index="d4_0")
            in_channels = [128, 128, 128]
            num_heads = [2, 2, 2]
            Ret_depths = [3, 3, 3]
            LK_depths = [(1,), (1,), (1,)]
            LK_dims = [(36,36), (36,36), (36,36)]
            kernel_sizes = [(5,), (5,), (5,)]
            groups = [(6,), (6,), (6,)]
            self.convs["AdaRM_d2_0"] = AdaRM(in_channels[0], num_heads[0], Ret_depths[0], LK_depths[0], LK_dims[0],
                                             kernel_sizes[0], groups[0], index="d2_0",DDW_k=7)
            self.convs["AdaRM_d2_1"] = AdaRM(in_channels[1], num_heads[1], Ret_depths[1], LK_depths[1], LK_dims[1],
                                             kernel_sizes[1], groups[1], index="d2_1",DDW_k=7)
            self.convs["AdaRM_d3_1"] = AdaRM(in_channels[2], num_heads[2], Ret_depths[2], LK_depths[2], LK_dims[2],
                                             kernel_sizes[2], groups[2], index="d3_1",DDW_k=7)
            # in_channels = [64,64,64,64]
            # num_heads = [1, 1, 1, 1]
            # Ret_depths = [2, 2, 2, 2]
            # LK_depths = [(1,2), (1,2), (1,2), (1,2)]
            # LK_dims = [(18,18), (18,18), (18,18), (18,18)]
            # kernel_sizes = [(9,3), (9,3), (9,3), (9,3)]
            # groups = [(3,3), (3,3), (3,3), (3,3)]
            # self.convs["AdaRM_d1_0"] = AdaRM(in_channels[0], num_heads[0], Ret_depths[0], LK_depths[0], LK_dims[0],
            #                                  kernel_sizes[0], groups[0], index="d1_0",DDW_k=7)
            # self.convs["AdaRM_d1_1"] = AdaRM(in_channels[1], num_heads[1], Ret_depths[1], LK_depths[1], LK_dims[1],
            #                                  kernel_sizes[1], groups[1], index="d1_1",DDW_k=7)
            # self.convs["AdaRM_d1_2"] = AdaRM(in_channels[2], num_heads[2], Ret_depths[2], LK_depths[2], LK_dims[2],
            #                                  kernel_sizes[2], groups[2], index="d1_2",DDW_k=7)
            # self.convs["AdaRM_d2_2"] = AdaRM(in_channels[3], num_heads[3], Ret_depths[3], LK_depths[3], LK_dims[3],
            #                                  kernel_sizes[3], groups[3], index="d2_2",DDW_k=7)
            # in_channels = [32, 32, 32, 32, 32]  # [18, 36, 72, 18, 36, 18, 64]
            # num_heads = [1, 1, 1, 1, 1]
            # Ret_depths = [2, 2, 2, 2, 2]
            # LK_depths = [(1,2), (1,2), (1,2), (1,2), (1,2)]
            # LK_dims = [(18,18), (18,18), (18,18), (18,18), (18,18)]
            # kernel_sizes = [(9,3), (9,3), (9,3), (9,3), (9,3)]
            # groups = [(3,3), (3,3), (3,3), (3,3), (3,3)]
            # self.convs["AdaRM_d0_0"] = AdaRM(in_channels[0], num_heads[0], Ret_depths[0], LK_depths[0], LK_dims[0],
            #                                  kernel_sizes[0], groups[0], index="d0_0")
            # self.convs["AdaRM_d0_1"] = AdaRM(in_channels[1], num_heads[1], Ret_depths[1], LK_depths[1], LK_dims[1],
            #                                  kernel_sizes[1], groups[1], index="d0_1")
            # self.convs["AdaRM_d0_2"] = AdaRM(in_channels[2], num_heads[2], Ret_depths[2], LK_depths[2], LK_dims[2],
            #                                  kernel_sizes[2], groups[2], index="d0_2")
            # self.convs["AdaRM_d0_3"] = AdaRM(in_channels[3], num_heads[3], Ret_depths[3], LK_depths[3], LK_dims[3],
            #                                  kernel_sizes[3], groups[3], index="d0_3")
            # self.convs["AdaRM_d1_3"] = AdaRM(in_channels[4], num_heads[4], Ret_depths[4], LK_depths[4], LK_dims[4],
            #                                  kernel_sizes[4], groups[4], index="d1_3")
        if self.use_channel_mamba_2:
            input_channels = ch_enc
            hide_channel = 20
            hide_channels = [4, 4, 4, 4, 4]
    ...
    def forward(self, input_features):
        ...
        for index in self.all_position:
            ...
            for i in range(col):
                if self.use_channel_mamba and index == "04" and i == 0:
                    features["X_{}{}".format(row, i)] = self.convs["X_00_Conv_extra"](features["X_{}{}".format(row, i)])
                if self.use_AdaRM and index != "04":
                # if self.use_AdaRM:
                    if index in ["31", "22"]:
            ...
            if index in self.attention_position:
                if self.use_channel_mamba:
                    # if self.use_AdaRM and index != "04":
                    #     features["X_{}{}".format(row + 1, col - 1)] = self.convs["AdaRM_d{}_{}".format(row + 1, col - 1)](features["X_{}{}".format(row + 1, col - 1)])
                    high_feature = upsample(self.convs["X_{}{}_Conv_0".format(row + 1, col - 1)](
                            features["X_{}{}".format(row + 1, col - 1)]))
                    if self.use_AdaRM and (index == "31" or index == "22"):
        ...
        x = self.convs["X_04_Conv_0"](x)
        x = self.convs["X_04_Conv_1"](upsample(x))
        outputs[("d_feature", 0)] = x
        outputs[("d_feature", 1)] = features["X_04"]
        outputs[("d_feature", 2)] = features["X_13"]
        outputs[("d_feature", 3)] = features["X_22"]
        outputs[("disp", 0)] = self.sigmoid(self.convs["dispconv0"](x))
        outputs[("disp", 1)] = self.sigmoid(self.convs["dispconv1"](features["X_04"]))
        outputs[("disp", 2)] = self.sigmoid(self.convs["dispconv2"](features["X_13"]))
        outputs[("disp", 3)] = self.sigmoid(self.convs["dispconv3"](features["X_22"]))
