...
# 所以from torch.utils.tensorboard import SummaryWriter和from tensorboardX import SummaryWriter只能写一个
...
class Trainer:
    def __init__(self, options):
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, 4, is_train=True, is_use_guide_map=self.opt.is_use_guide_map, img_ext=img_ext)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        val_dataset = self.dataset(
            self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, 4, is_train=False, is_use_guide_map=self.opt.is_use_guide_map, img_ext=img_ext)
    ...
    def compute_reprojection_L1loss(self, pred, target):

        abs_diff = torch.abs(target - pred)
        l1_loss = abs_diff.mean(1, True)

        reprojection_loss = 0.15 * l1_loss

        return reprojection_loss

    def compute_reprojection_SSIMloss(self, pred, target):

        ssim_loss = self.ssim(pred, target).mean(1, True)
        reprojection_loss = 0.85 * ssim_loss
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.is_use_guide_map and self.start_guide and (self.start_Res_smooth_guide or self.start_smooth_guide):
                if scale == 0:
                    ...
                    # Res_guide_mask1是12, 1, 192, 640的矩阵
                    # 先计算原来的最小重投影损失
                for frame_id in self.opt.frame_ids[1:]:
                    pred = outputs[("color", frame_id, scale)]
                    outputs[("L1loss", frame_id, scale)] = self.compute_reprojection_L1loss(pred, target)
                    outputs[("SSIMloss", frame_id, scale)] = self.compute_reprojection_SSIMloss(pred, target)
                    # outputs[("SSIMloss", frame_id, scale)]的尺寸为torch.Size([12, 1, 192, 640])
